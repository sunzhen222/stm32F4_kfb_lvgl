...
void demo_create(void)
{
	...
	//style_tv_btn_bg.body.main_color = lv_color_hex(0xFF7fb7);
	//style_tv_btn_bg.body.grad_color = lv_color_hex(0xFF7fb7);
...
static void list_create(lv_obj_t * parent)
{
	...
	list_btn = lv_list_add_btn(list, "\xff\x00\xff", "New~~");
	...
	lv_obj_t * mbox2 = lv_mbox_create(parent, NULL);
	lv_mbox_set_text(mbox2, "This is another btn");
	lv_obj_set_width(mbox2, hres - LV_DPI*2);
	static const char * mbox_btns2[] = { "Click me", "" };
	lv_mbox_add_btns(mbox2, mbox_btns2);    /*The default action is close*/
	lv_obj_align(mbox2, parent, LV_ALIGN_IN_TOP_MID, 0, 0);
	...
	static lv_style_t style_testbtn_rel; /*A variable to store the released style*/
	lv_style_copy(&style_testbtn_rel, &lv_style_plain); /*Initialize from a built-in style*/
	style_testbtn_rel.body.border.color = lv_color_hex3(0x269);
	style_testbtn_rel.body.border.width = 1;
	style_testbtn_rel.body.main_color = lv_color_hex3(0xADF);
	style_testbtn_rel.body.grad_color = lv_color_hex3(0x46B);
	style_testbtn_rel.body.shadow.width = 4;
	style_testbtn_rel.body.shadow.type = LV_SHADOW_BOTTOM;
	style_testbtn_rel.body.radius = 10;//LV_RADIUS_CIRCLE;
	style_testbtn_rel.text.color = lv_color_hex3(0xDEF);


	static lv_style_t style_testbtn_pr; /*A variable to store the pressed style*/
	lv_style_copy(&style_testbtn_pr, &style_testbtn_rel); /*Initialize from the released style*/
	style_testbtn_pr.body.border.color = lv_color_hex3(0x46B);
	style_testbtn_pr.body.main_color = lv_color_hex3(0x8BD);
	style_testbtn_pr.body.grad_color = lv_color_hex3(0x24A);
	style_testbtn_pr.body.shadow.width = 2;
	//style_testbtn_pr.text.color = lv_color_hex3(0xBCD);
	style_testbtn_pr.text.color = lv_color_hex3(0xFFF);


	lv_btn_set_style(btn, LV_BTN_STYLE_REL, &style_testbtn_rel); /*Set the button's released style*/
	lv_btn_set_style(btn, LV_BTN_STYLE_PR, &style_testbtn_pr); /*Set the button's pressed style*/
